...
Datum gpsc_stat_messages_reset(PG_FUNCTION_ARGS) {
  gpsc_functions_reset();
  PG_RETURN_VOID();
}

Datum gpsc_stat_messages(PG_FUNCTION_ARGS) {
  return gpsc_functions_get(fcinfo);
}

Datum gpsc_init_log(PG_FUNCTION_ARGS) {
  init_log();
  PG_RETURN_VOID();
}

Datum gpsc_truncate_log(PG_FUNCTION_ARGS) {
  truncate_log();
  PG_RETURN_VOID();
}

Datum gpsc_test_uds_start_server(PG_FUNCTION_ARGS) {
  ...
  gpsc_test_uds_stop_server(NULL);
...
Datum gpsc_test_uds_receive(PG_FUNCTION_ARGS) {
...
Datum gpsc_test_uds_stop_server(PG_FUNCTION_ARGS) {
  if (server_fd >= 0) {
    close(server_fd);
    server_fd = -1;
  }
