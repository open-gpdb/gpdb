...
-- Cross-slice ShareInputScan wait accounting.
--
-- A consumer that blocks waiting for a slow producer slice reports the wait
-- in EXPLAIN ANALYZE ("Cross-slice wait: N ms max ..."). Force a
-- deterministic, above-threshold wait by making the producer sleep, and
-- assert the line appears. The wrapper hides the non-deterministic timing
-- value, returning only whether the line is present, so the test is stable.
...
analyze sisw_foo;
analyze sisw_bar;
create function sisw_wait_reported(q text) returns boolean as $fn$
declare ln text;
begin
  for ln in execute 'explain (analyze, timing off, costs off) ' || q loop
    if strpos(ln, 'Cross-slice wait:') > 0 then return true; end if;
  end loop;
  return false;
end; $fn$ language plpgsql;
set optimizer = off;
set gp_cte_sharing = on;
select sisw_wait_reported($q$
...
reset optimizer;
drop function sisw_wait_reported(text);
drop table sisw_foo, sisw_bar;
