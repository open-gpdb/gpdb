		...
		case RTE_TEMPRESULT:
			{
				 ...
				 */
				TempResultEntry *tre = TempResultLookupId(rte->ctename,
														  rte->tempresid);
				...
				/* without a policy the locus would silently become Entry */
				if (tre->policy == NULL)
					elog(ERROR, "catalogless temp table \"%s\" has no distribution policy",
						 rte->ctename);
