...
ExecInitCustomScan(CustomScan *cscan, EState *estate, int eflags)
{
	...
	/*
	 * Initialize scan slot and result slot tuple table slots.
	 */
	...
	/*
	 * For projections, if the custom scan tlist is set, we project from
	 * INDEX_VAR; otherwise use the scan relation.
	 */
...
ExecCustomRestrPos(CustomScanState *node)
{
	if (!node->methods->RestrPosCustomScan)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("custom scan \"%s\" does not support RestrPos",
						node->methods->CustomName)));
