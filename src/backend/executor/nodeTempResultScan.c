...
ExecEndTempResultScan(TempResultScanState *node)
{
	...
	/*
	 * Close the reader store now rather than at end of transaction, so that
	 * a transaction running many scans does not accumulate open files and
	 * page buffers.  Readers never delete the files; the writer does.
	 */
	if (node->ts_store)
	{
		TempResultCloseReader(node->ts_store);
		node->ts_store = NULL;
	}
