...
SH_COMPUTE_PARAMETERS(SH_TYPE * tb, uint32 newsize)
{
	...
	if ((((uint64) sizeof(SH_ELEMENT_TYPE)) * size) >= SIZE_MAX / 2)
		exit_horribly(NULL,"hash table too large\n");
...
restart:
	...
	if (unlikely(tb->members >= tb->grow_threshold))
	{
		if (tb->size == SH_MAX_SIZE)
		{
			exit_horribly(NULL,"hash table size exceeded\n");
		}
...
SH_STAT(SH_TYPE * tb)
{
	...
	write_msg(NULL, "size: " UINT64_FORMAT ", members: %u, filled: %f, total chain: %u, max chain: %u, avg chain: %f, total_collisions: %u, max_collisions: %i, avg_collisions: %f\n",
