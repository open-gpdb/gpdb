 ...
 *		CustomScan node
 *
 * The comments for ForeignScan's fdw_exprs/fdw_private fields apply equally
 * to custom_exprs and custom_private.  Note that since Plan trees can be
 * copied, custom scan providers *must* fit all plan data into those fields;
 * embedding CustomScan in a larger struct will not survive copyObject().
 * ----------------
 */
...
typedef struct CustomScan
{
	...
	uint32		flags;				/* mask of CUSTOMPATH_* flags, see
									 * nodes/extensible.h */
	List	   *custom_plans;		/* list of Plan trees */
	List	   *custom_exprs;		/* expressions that custom code may
									 * evaluate */
